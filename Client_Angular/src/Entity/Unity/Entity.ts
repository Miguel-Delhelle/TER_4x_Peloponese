...
export class Entity{
  ...
  constructor(id:string){
    this.id = id;
    this.graphicEntity = this.initGraphicalElement();
    this.initListener();
    //this.scaleByTile();
  }
  ...
  private scaleByTile(): void {
    /*let icr: number = map.$tilePolygon.$innerCircleRadius;
    //let w: number = this.graphicEntity.getAttribute("width");
    //let h: number = this.graphicEntity.getAttribute("height");
    let k: number = (1.8*icr)/(Math.sqrt(Math.pow(w,2)+Math.pow(h,2)));
    this.graphicEntity.setAttribute("width",w*k);
    this.graphicEntity.setAttribute("height",h*k); */
  }
